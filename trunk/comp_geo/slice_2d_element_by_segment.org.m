...
if (elemi == cuteleminfo.elemno)
      ...
      if (size(tri_bel,1) == 1)
          CONN(1:3,maxelemid+1) = xptoX_bel(tri_bel(1,1:3));
    ...
    elseif (nb_int == 1)
       ...
       elseif (edge_ids == [2 3])
          ...
          elemi
          edge_ids
          error('slice_2d_element_by_segment4','The edge_ids are not correct');
       end
       %generate the new subelements
       tri_abo = delaunay(xp_abo,yp_abo);
       tri_bel = delaunay(xp_bel, yp_bel);
       ...
       if (size(tri_abo,1) == 1)
          ...
          tri_abo
          error('slice_2d_element_by_segment5','The above triangulation failed');
       end
       if (size(tri_bel,1) == 2)
          CONN(1:3,maxelemid+2) = xptoX_bel(tri_bel(1,1:3));
          CONN(1:3,maxelemid+3) = xptoX_bel(tri_bel(2,1:3));
          ...
          tri_bel
          error('slice_2d_element_by_segment6','The below triangulation failed');
       end
       nb_sub_elts = 3;
       sub_elt_ids = [maxelemid+1 maxelemid+2 maxelemid+3];
       %plot the elements if debug
