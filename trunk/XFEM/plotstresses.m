...
% scale the colormap
V = [minstress_vonmises maxstress_vonmises];
colormap(jet(100));
caxis(V);
colorbar;
...
for e = 1:numele
    ...
    if cutlist(e) == 0
        ...
        % plot colored element
        if plotedges == 1
            patch(minix,miniy,stressvonmises(e,4,grain));
        else
            patch(minix,miniy,stressvonmises(e,4,grain),'EdgeColor','none');
        end;
    end;
end;
...
text_vonmises = ['Max. von-Mises-stress:  ' num2str(maxstress_vonmises) ...
    '   Min. von-Mises-stress:    ' num2str(minstress_vonmises)];
disp(text_xx);
disp(text_yy);
disp(text_xy);
disp(text_vonmises);
