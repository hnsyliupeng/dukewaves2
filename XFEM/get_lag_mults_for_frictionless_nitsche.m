% get_lag_mults_for_frictionless_nitsche.m
%
...
%   normal          normal vector of current subsegment
%
% Returned variables
...
function [lagmult] = get_lag_mults_for_frictionless_nitsche(node,x,y,parent,id_eqns, ...
    id_dof,pn_nodes,pos_g,neg_g,intersection,endpoints,penalty,fdisp,normal)
...
%--------------------------------------------------------------------------
% dot shape function matrix 'N' with normal vector to enforce constraints
% only in normal direction
N = normal' * N;

% compute lagrange multipliers (only in normal direction)
lagmult = penalty * N * localdis; % scalar value (in normal direction)
