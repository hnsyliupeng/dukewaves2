...
for j = 1:numele
    ...
    if (cutlist(j) == 0) % If element is uncut
        ...
        for i = 1:12
            ...
            dy_feta = dy_feta + 0.01;
    
            % x and y as a function of eta (for the analytical solution)
            x_feta = N1*xcoords(1) + N2*xcoords(2) + N3*xcoords(3);
            y_feta = N1*ycoords(1) + N2*ycoords(2) + N3*ycoords(3) - 0.5;
            ...
            if y_feta > - 2   % check, in which grain the point resides
              ...
              % find node, which is the nearest to current gauss point
              % Difference-vecotr
              diff_x = x_ref - x_feta;
              diff_y = y_ref - y_feta;
  ...
  %             approx_solny = approx_solny + (dy_feta)^2*wg(i)*jcob;
    ...
    elseif (cutlist(j) ~= 0) % If element is cut
        ...
        for m = 1:SUBELEM_INFO(j).no_kids % loop through subelements
            ...
            subele = SUBELEM_INFO(j).kids(m); % global ID of subelement
...
%                 yd = dispy;%dis(2*nodes);    
                ...
                % element displacement vector
                if id_dof(elenodes(1),3) == 1%grain
                  for b=1:3
                    % extra degrees of freedom
                    b1 = id_eqns(elenodes(b),3);
                    b2 = id_eqns(elenodes(b),4); 
                ...
                if id_dof(elenodes(1),5) == 1%grain
                  for b=1:3
                    % extra degrees of freedom
                    b1 = id_eqns(elenodes(b),5);
                    b2 = id_eqns(elenodes(b),6); 
                ...
                % Interpolate displacements of current subelement's nodes
                for c=1:3 % loop over subelement nodes
                  N_sub = zeros(1,3);  
                  for d=1:3
                    xes = xcoords;
                    yes = ycoords;
                  ...
                  % interpolate displacement at subelement node
                  xds(c) = N_sub * xd';
                  yds(c) = N_sub * yd';
                end;
                ...
                for i = 1:12
                    ...
                    % The displacements as a function of r and s
                    dx_feta = N1*xds(1) + N2*xds(2) + N3*xds(3);
                    dy_feta = N1*yds(1) + N2*yds(2) + N3*yds(3);
...
% L2norm_x = sqrt((total_errorx)/(total_apprx))
% L2norm_y = sqrt((total_errory)/(total_appry))
